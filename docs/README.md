# Sistema de Controle de Despesas - Etapa B4T01.1

## 🧾 Versão 0.0.1
**Descrição:** Repositório criado, clonado, menu feito e println de cada funcionalidade ao acessar o menu.  
**Status:** ✅ Concluído  

---

## 📄 Changelog da Entrega
| Etapa | Versão | Descrição | Data |
|--------|---------|------------|------|
| B4T01.1 | 0.0.1 | Repositório criado, clonado, menu feito e println de cada funcionalidade ao acessar o menu. | *(03/11/2025)* |

---

## 📁 Estrutura do Projeto
b4t1_trabalho/
├── src/
│ └── app/
│ └── Main.java
└── docs/
└── README.md

---

## 🧠 Próximos Passos
- Implementar armazenamento em arquivo texto  
- Criar classes para Despesa, Pagamento, TipoDespesa e Usuário  
- Aplicar herança, polimorfismo e interfaces conforme enunciado  

---

## 🧾 Versão 0.0.2
**Descrição:** Estrutura de classes criada e menu principal funcional.  
**Status:** ✅ Concluído  

| Versão | Descrição                                         | Data       |
| ------ | ------------------------------------------------- | ---------- |
| 0.0.1  | Repositório criado e menu principal com `println` | 03/11/2025 |
| 0.0.2  | Estrutura de classes criada e menu funcional      | 09/11/2025 |


---

### 🧠 Próximos Passos
- Implementar salvamento de dados em arquivos `.txt`  
- Adicionar funcionalidades reais de cadastro e listagem  
- Aplicar herança, polimorfismo e interface Pagável  
